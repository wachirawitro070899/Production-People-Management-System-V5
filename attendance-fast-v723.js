...
(()=>{
 ...
 assertAndBindAttendanceDeviceCloud=async function(emp){
  ...
  let tokenRows={};
  let deviceRoot=ATTENDANCE_CLOUD_ROOT+'/devices';
  try{tokenRows=decode(await wait(cloudDb.ref(deviceRoot).orderByChild('token').equalTo(token).once('value'),4500,'ตรวจสอบการผูกเครื่อง'))||{}}
  catch(error){
   console.warn('V723 primary token lookup unavailable',error);
   if(denied(error)){
    deviceRoot='ppms/attendanceDevices';
    try{tokenRows=decode(await wait(cloudDb.ref(deviceRoot).orderByChild('token').equalTo(token).once('value'),3500,'ตรวจสอบการผูกเครื่องสำรอง'))||{}}
    catch(fallbackError){console.warn('V737 fallback token lookup unavailable',fallbackError);throw error}
   }
  }
  ...
  let ref=cloudDb.ref(deviceRoot+'/'+firebaseEncodeKey(empId));let own=null;
  try{own=decode(await wait(ref.once('value'),3500,'อ่านข้อมูลเครื่อง'))||null}catch(_){own=attendanceDevices[empId]||null}
  const now=new Date().toISOString(),migrating=!!(own&&String(own.token||'')!==token),next={...(own||{}),token,label:deviceLabel(),registeredAt:own?.registeredAt||now,lastSeenAt:now,lockVersion:deviceRoot==='ppms/attendanceDevices'?'V737-legacy-fallback':'V737'};
  ...
  try{await wait(ref.set(firebaseEncodeData(next)),5000,'บันทึกข้อมูลเครื่อง')}
  catch(error){
   if(!denied(error)||deviceRoot==='ppms/attendanceDevices')throw error;
   deviceRoot='ppms/attendanceDevices';ref=cloudDb.ref(deviceRoot+'/'+firebaseEncodeKey(empId));next.lockVersion='V737-legacy-fallback';
   await wait(ref.set(firebaseEncodeData(next)),5000,'บันทึกข้อมูลเครื่องสำรอง');
  }
