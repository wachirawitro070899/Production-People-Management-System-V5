...
(async()=>{
assert(ctx.biometricIntegrationPanel().includes('อัปโหลดข้อมูลการสแกน'));ctx.biometricSettingsModal();assert(html.includes('biometricSettingsForm'));await form.onsubmit({preventDefault(){}});assert.equal(saved.value.enabled,false);assert.equal(saved.value.status,'draft');assert.equal(saved.path,'ppmsAttendance/settings/biometricIntegration');assert.equal(ctx.attendanceSettings.biometricIntegration.deviceId,'JR-GATE-01');assert(message.textContent.includes('บันทึกข้อมูลเครื่องแล้ว'));assert.equal(button.disabled,false);
